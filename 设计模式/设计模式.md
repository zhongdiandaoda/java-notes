...
结构性模式：通过识别系统中组件间的简单关系来简化系统的设计。

创建型模式：处理对象的创建，根据实际情况使用合适的方式创建对象。常规的对象创建方式可能会导致设计上的问题，或增加设计的复杂度。

行为型模式：用于识别对象之间常见的交互模式并加以实现，增加了交互的灵活性。
...
但是即使 uniqueInstance 已经被实例化了，当一个线程进入该方法之后，其它试图进入该方法的线程都必须等待，这会让线程阻塞时间过长，因此该方法有性能问题，不推荐使用。
...
uniqueInstance 采用 volatile 关键字修饰也是很有必要的， `uniqueInstance = new Singleton();` 这段代码的字节码为：
...
putstatic指令将对象地址赋值给变量t，这时也会弹出一个栈顶元素
invokespecial指令与putstatic指令发生重排序时，会使这个INSTANTCE进行了半初始化时被其他线程获取，导致程序出现错误。

### Ⅴ 静态内部类实现

当 Singleton 类被加载时，静态内部类 SingletonHolder 没有被加载进内存。只有当调用 `getUniqueInstance()` 方法从而触发 `SingletonHolder.INSTANCE` 时 SingletonHolder 才会被加载，此时初始化 INSTANCE 实例，并且 JVM 能确保 INSTANCE 只被实例化一次。

这种方式不仅具有延迟初始化的好处，而且由 JVM 提供了对线程安全的支持，是非常推荐使用的一种单例模式。
...
![img](%E8%AE%BE%E8%AE%A1%E6%A8%A1%E5%BC%8F.assets/218fadcef60485a10c7b72b5f138e970.jpeg)

**优点：** 

1、满足单一职责，每个业务逻辑都在自己所属的子类中完成。
...
**缺点：**产品族扩展非常困难，要增加一个系列的某一产品，既要在抽象的 Creator 里加代码，又要在具体的实现里面加代码。
...
public class BigFactory implements AbstractFactory {
    ...
    public String createHtml(String md) {
        return new FastHtmlDocumentImpl().getHtmlRes();
    }

    @Override
    public String createWord(String md) {
        return new FastWordDocumentImpl().getWordRes();
    }
}
```
...
public class SmallFactory implements AbstractFactory {
    @Override
    public String createHtml(String md) {
        return new SlowHtmlDocumentImpl().getHtmlRes();
    }

    @Override
    public String createWord(String md) {
        return new SlowWordDocumentImpl().getWordRes();
    }
}
```

3. 两个工厂使用不同方法实现抽象工厂类的方法
...
```java
public class SlowWordDocumentImpl implements WordDocument {
    @Override
    public String getWordRes() {
        return "小工厂的慢速Word文档";
    }
}


public class SlowHtmlDocumentImpl implements HtmlDocument {
    @Override
...
public class Test {
    public static void main(String[] args) {
        AbstractFactory abstractFactory = AbstractFactory.createFactory("fast");
        System.out.println(abstractFactory.createHtml("asd"));
        System.out.println(abstractFactory.createWord("asd"));

        AbstractFactory abstractFactory1 = AbstractFactory.createFactory("slow");
        System.out.println(abstractFactory1.createWord("asd"));
        System.out.println(abstractFactory1.createHtml("asd"));
    }
}
```
...
1、实现克隆操作，在 JAVA 继承 Cloneable，重写 clone()。非基本类型的成员需要手动实现clone操作。
...
![原型模式的 UML 图](设计模式.assets/20201202-prototype-pattern.png)
...
- 将代理对象与被调用对象分离，降低了系统的耦合度，可以起到保护目标对象的作用。
- 代理对象可以扩展目标对象的功能；需要修改业务时，修改代理对象的代码就可以了，符合开闭原则。
